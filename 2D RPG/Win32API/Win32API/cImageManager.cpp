...
void cImageManager::FrameRenderMinus(string key, HDC hdc, int destX, int destY, int sourX, int sourY,
	int minX, int maxY, int delay)
{
	cImage* img = FindImage(key);
	if (img) img->FrameRenderMinus(hdc, destX, destY, sourX, sourY, minX, maxY, delay);
}

void cImageManager::FrameSizeRender(string key, HDC hdc, int destX, int destY, int width, int height)
{
	cImage* img = FindImage(key);
	if (img) img->FrameSizeRender(hdc, destX, destY, width, height);
}

void cImageManager::FrameSizeRender(string key, HDC hdc, int destX, int destY, int width, int height, int sourX, int sourY)
{
	cImage* img = FindImage(key);
	if (img) img->FrameSizeRender(hdc, destX, destY, width, height, sourX, sourY);
}

void cImageManager::FrameSizeRender(string key, HDC hdc, int destX, int destY, int width, int height, int sourX, int sourY, int maxX, int maxY, int delay)
{
	cImage* img = FindImage(key);
	if (img) img->FrameSizeRender(hdc, destX, destY, width, height, sourX, sourY, maxX, maxY, delay);
}

void cImageManager::ViewPortRender(string Key, HDC hdc, RECT ViewPort)
{
	cImage* img = FindImage(Key);
	if (img) img->ViewPortRender(hdc, ViewPort);
}
