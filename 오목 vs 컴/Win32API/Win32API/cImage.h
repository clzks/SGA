...
public:
	...
	void AlphaRender(HDC hdc, int destX, int destY, int sourX, int sourY, BYTE alpha);
	...
	void FrameRenderMinus(HDC hdc, int destX, int destY,
		int sourX, int sourY, int minX, int maxY, int delay);
	void FrameSizeRender(HDC hdc, int destX, int destY, int width, int height);
	void FrameSizeRender(HDC hdc, int destX, int destY, int width, int height, int sourX, int sourY);
	void FrameSizeRender(HDC hdc, int destX, int destY,
	...
	RECT GetBoundingBox() // 레프트, 탑으로 만들기
	{
		return RectMake(m_pImageInfo->fPosX, m_pImageInfo->fPosY,
