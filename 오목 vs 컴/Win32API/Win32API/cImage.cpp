...
void cImage::AlphaRender(HDC hdc, int destX, int destY, int sourX, int sourY, BYTE alpha)
{
	// 알파블렌드 처음 사용시 초기화
	if (!m_pBlendImage) SetupForAlphaBlend();
	...
	if (m_isTrans)
	{
		...
		GdiTransparentBlt(m_pBlendImage->hMemDC, 0, 0, m_pImageInfo->nWidth, m_pImageInfo->nHeight,
			m_pImageInfo->hMemDC, 0, 0, m_pImageInfo->nWidth, m_pImageInfo->nHeight, m_transColor);
		//3. 블렌드 DC를 출력해야 할 DC에 그린다
		GdiAlphaBlend(hdc, destX, destY, sourX, sourY,
...
void cImage::FrameRenderMinus(HDC hdc, int destX, int destY,
	int sourX, int sourY, int minX, int maxY, int delay)
{
	if (m_pImageInfo->nDelay >= delay)
	{
		...
		if (m_pImageInfo->nCurrFrameX <= minX)
			m_pImageInfo->nCurrFrameX = sourX;
		else
			--m_pImageInfo->nCurrFrameX;
...
void cImage::FrameSizeRender(HDC hdc, int destX, int destY, int width, int height, int sourX, int sourY)
{
	if (m_isTrans)   // 배경색을 없앨 경우
	{
		// GdiTransparentBlt : 비트맵을 불러올때 특정색상을 제외하고 복사를 하는 함수
...
void cImage::FrameSizeRender(HDC hdc, int destX, int destY,
